...
const DETAIL_FOCUS_OPTIONS_SYSTEM_PROMPT = `你是一位高端服装电商视觉策划，正在帮店主为一件单品挑选“最值得拍的细节特写”。

你会同时看到：
1. 用户上传的原始单品参考图
2. 已生成好的主图

你的任务不是直接生成图片，而是先给用户一组“可以点击选择”的细节候选项。

只返回合法 JSON，不要 markdown，不要解释文字。所有内容用中文。

## 核心规则
1. 只能基于图片里真实可见、真实存在的细节给建议，绝对不能臆造不存在的纽扣、拉链、口袋、刺绣、压花、拼接、五金或装饰。
2. 候选项要覆盖“结构 / 元素 / 工艺 / 面料”等不同角度，避免多个建议过于重复。
3. 每个建议都要适合被拍成一张近景细节图，不能写成“整体氛围”或“高级感”这种泛描述。
4. instruction 要写成给图像模型的清晰指令，必须强调：
   - 仍然是同一件单品
   - 只拍真实存在的局部
   - 必须是近景特写
   - 保持与主图一致的光线和调性
5. 如果某类细节不明显，就换成别的真实细节，不要为了凑数硬写。

## 输出要求
- suggestions 返回 5 到 8 个候选项
- category 只能是：structure、feature、craft、fabric
- title 要短，适合前端卡片展示
- reason 用一句话解释为什么值得拍
- priority 用数字表示推荐强度，越大越优先

返回 JSON：
{
  "itemType": "单品类型，如'短款牛仔外套'或'高腰阔腿裤'",
  "summary": "一句话概括这件衣服最值得拍什么细节",
  "suggestions": [
    {
      "id": "structure-collar",
      "category": "structure",
      "title": "领口与门襟结构",
      "instruction": "生成一张近景细节特写图，聚焦这件衣服真实存在的领口和门襟结构，保持同一件单品、同一光线调性，禁止拍出不存在的元素。",
      "reason": "这部分最能说明版型和做工。",
      "priority": 95
    }
  ]
...
serve(async (req) => {
  ...
  try {
    ...
    // ========== 细节特写候选分析模式 ==========
    if (mode === 'detail-focus-options') {
      if (!TEXT_API_KEY) {
        throw new Error('BLTCY_TEXT_API_KEY not configured for detail-focus-options mode')
      }
      ...
      const detailFocusMessages = [
        { role: 'system', content: DETAIL_FOCUS_OPTIONS_SYSTEM_PROMPT },
        { role: 'user', content: userContent },
      ]

      const detailFocusResponse = await fetch(`${TEXT_BASE_URL}/v1/chat/completions`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${TEXT_API_KEY}`,
      ...
      if (!detailFocusResponse.ok) {
        const errorText = await detailFocusResponse.text()
        throw new Error(`Detail Focus Options error: ${detailFocusResponse.status} - ${errorText}`)
      }

      const detailFocusData = await detailFocusResponse.json()
      requireNonEmptyContent(
        extractAssistantMessage(detailFocusData),
        'Detail Focus Options returned empty content',
      )
      return new Response(JSON.stringify(detailFocusData), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
