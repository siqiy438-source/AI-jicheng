import { FASHION_EXPOSURE_SAFETY_RULES_EN, FASHION_EXPOSURE_SAFETY_RULES_ZH } from "@/lib/fashion-safety";
...
export const FASHION_OUTFIT_COMBO_PROMPT = `创建一张完整穿搭组合展示图(outfit combo flat lay)，将一套搭配的所有单品按分区布局整齐排列，呈现"今日穿搭推荐"的效果。

【核心布局规则 - 必须严格遵守】
- 画面分为左右两大区域：
  · 左侧区域（约占画面40%宽度）：从上到下依次放置帽子/围巾 → 内搭上衣（叠好的状态）→ 裤子（叠好的状态）→ 鞋子（一双并排摆放）
  · 右侧区域（约占画面55%宽度）：放置外套/大衣/夹克（完整展开铺平，从上到下占据右侧大部分空间）
  · 包包放在左下方或右下方，与鞋子同一水平线附近
- 所有服装必须是叠好或铺平的状态，不是穿在人身上，不是挂在衣架上
- 内搭上衣和裤子呈整齐叠放状态（像店铺陈列那样叠好），不是完全展开的
- 外套/大衣在右侧完整展开铺平，展示全貌，可以自然地有些褶皱
- 单品之间可以自然靠近，边缘可以微微接触，但不要大面积重叠

【搭配逻辑 - 智能补充】
- 分析上传服装的风格、色系和季节感，组成一套完整和谐的穿搭方案
- 根据已上传的服装，智能添加缺少的单品来完善整套搭配：
  · 一双合适的鞋子（切尔西靴、马丁靴、运动鞋、板鞋等，与整体风格协调）
  · 一个包包（托特包、斜挎包、腰包等，与风格匹配）
  · 1件配饰（毛线帽、棒球帽、围巾等，选择最能提升整体感的）
- 如果上传的服装已经包含外套，则右侧放该外套；如果没有外套，可以智能推荐一件与已有单品风格协调的外套
- 整套搭配的色彩必须和谐统一，有明确的色彩主题

【摆放风格 - 关键特征】
- 每件单品摆放工整、有序，像是博主精心摆好准备拍照的状态
- 上衣和裤子是叠好的状态，展示正面，能看到领口、口袋等关键细节
- 外套自然展开，可以稍微露出内衬颜色
- 鞋子干净整洁，一左一右自然并排
- 包包自然放置，包带可以自然弯曲延伸
- 帽子/围巾等配饰放在对应位置，增加搭配的完整感

【背景与拍摄】
- 俯拍视角（正上方90度鸟瞰角度）
- 背景为浅灰色或米色的水泥/石材纹理质感表面，有细微的肌理但不抢眼
- 背景干净无杂物，只有服装和配饰
- 均匀柔和的自然光线，每件单品有轻微的自然投影
- 整体画面干净、高级、有质感

【技术要求】
1. 照片级真实感，面料质感清晰可辨
2. 每件单品的颜色、款式、面料纹理必须与上传图片高度一致
3. 无文字、无水印、无标签、无价格标注
4. 竖版 9:16 比例
5. 无模特、无人台、无衣架
6. 整体构图平衡美观，留白适当

风格参考：小红书穿搭博主的"今日穿搭分享"平铺图、Instagram outfit grid、Pinterest outfit inspiration flat lay`;
...
${FASHION_EXPOSURE_SAFETY_RULES_ZH}
...
NO: eyes, nose, full face, stiff fingers, deformed hands, over-smoothed skin, plastic-looking fabrics, bright saturated colors, cluttered background, full body shot, extra clothing, invented outerwear, non-East-Asian identity, nudity, near-nudity, exposed nipples, exposed areola, exposed underbust, exposed genital area, butt crack, see-through private parts
...
NO: eyes, nose, face, full body, wide framing, distant camera, knees, legs, feet, shoes, stiff fingers, plastic fabrics, bright colors, cluttered background, flat lighting, extra clothing, invented outerwear, non-East-Asian identity, nudity, near-nudity, exposed nipples, exposed areola, exposed underbust, exposed genital area, butt crack, see-through private parts
