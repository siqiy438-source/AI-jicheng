...
export interface BuildHangoutfitWorkMetadataParams {
  ...
  hasShoesImage: boolean;
  hasBagImage: boolean;
  detailStripCount?: number;
  sentReferenceImageCount?: number;
  fidelityMode?: string;
}

function buildAccessorySection(accessories?: AccessoryUploadInfo, uploadedCount: number = 0): string {
  const hasShoes = accessories?.hasShoesImage ?? false;
  const hasBag = accessories?.hasBagImage ?? false;
  const firstAccessoryImageIndex = uploadedCount * 2 + 2;

  let imageIndexNote = "";
  if (hasShoes && hasBag) {
    imageIndexNote = `\n\n[IMPORTANT: Accessory Reference Images]\nYou are given additional separate reference images after the garment detail strips:\n- Image ${firstAccessoryImageIndex} is a close-up photo of the SHOES to use. Reproduce this exact pair of shoes.\n- Image ${firstAccessoryImageIndex + 1} is a close-up photo of the BAG to use. Reproduce this exact handbag.\nThese are high-resolution accessory references. Copy them exactly.`;
  } else if (hasShoes) {
    imageIndexNote = `\n\n[IMPORTANT: Accessory Reference Image]\nYou are given an additional separate reference image after the garment detail strips:\n- Image ${firstAccessoryImageIndex} is a close-up photo of the SHOES to use. Reproduce this exact pair of shoes.\nThis is a high-resolution accessory reference. Copy it exactly.`;
  } else if (hasBag) {
    imageIndexNote = `\n\n[IMPORTANT: Accessory Reference Image]\nYou are given an additional separate reference image after the garment detail strips:\n- Image ${firstAccessoryImageIndex} is a close-up photo of the BAG to use. Reproduce this exact handbag.\nThis is a high-resolution accessory reference. Copy it exactly.`;
  }
...
function buildDetailGuardrails(): string {
  return `[Critical Small-Detail Fidelity — Highest Priority]
Copy all visible garment details exactly as shown in the user's uploaded garment photos. This includes:
- exact button count, button placement, spacing, and visible metal finish
- exact zipper/placket shape and closure construction
- exact waistband construction, including double-button or offset-button closures when visible
- exact pocket count, pocket angle, flap/no-flap structure, welt lines, and seam placements
- exact collar, neckline, lapel, bow, tie, ruffle, pleat, cuff, and hem finish
- exact print scale, print density, print placement, fading pattern, wash pattern, and panel lines
- exact raw hems, fringe, distressed edges, topstitching, labels, tabs, and trims
Never simplify a visible small detail into a generic version. Example: if the reference shows two visible waistband buttons, the final garment must also show two visible waistband buttons, not one button and not a hidden closure.`;
}

function buildSinglePassReferenceMap(
  uploadedCount: number,
  boardLabel: string,
  accessories?: AccessoryUploadInfo,
): string {
  const fullStart = 2;
  const fullEnd = uploadedCount + 1;
  const detailStart = uploadedCount + 2;
  const detailEnd = uploadedCount * 2 + 1;
  const accessoryNotes: string[] = [];
  let nextImageIndex = detailEnd + 1;

  if (accessories?.hasShoesImage) {
    accessoryNotes.push(`- Image ${nextImageIndex}: exact SHOES reference photo provided by the user. Reproduce this pair faithfully if shoes appear.`);
    nextImageIndex += 1;
  }
  if (accessories?.hasBagImage) {
    accessoryNotes.push(`- Image ${nextImageIndex}: exact BAG reference photo provided by the user. Reproduce this bag faithfully if the bag appears.`);
  }

  return `[Reference Image Order & Priority]
You will receive multiple reference images in this exact order:
- Image 1: ${boardLabel}
- Images ${fullStart}-${fullEnd}: the user's original full garment photos for garments #1-${uploadedCount}. These are the primary truth source for overall garment identity.
- Images ${detailStart}-${detailEnd}: enlarged detail-strip references for garments #1-${uploadedCount}, in the same order. Each strip shows upper, middle, and lower close views to preserve small visible details.${accessoryNotes.length > 0 ? `\n${accessoryNotes.join('\n')}` : ''}

Priority order when references conflict:
1. The garment's own full photo plus its matching detail strip
2. The garment overview tiles in Image 1
3. The scene/template portion of Image 1
4. Additional user notes

Scene templates may define wall, rod, props, lighting, and composition, but they must NEVER change the garment itself. If the template scene suggests a simpler closure, different collar, different print, or fewer details than the user's garment photo, ignore the template and copy the user's garment exactly.`;
}
...
${buildSinglePassReferenceMap(
    uploadedCount,
    "an overview garment board showing all uploaded garments together. Use it only as a garment index and count check, not as the highest-detail source.",
    accessories,
  )}

[Instruction: Input Garment & Realism Patch]
Using the user's uploaded garment photos, display the garments with natural gravity and relaxed drape. Show realistic fabric weight, gentle natural wrinkles, and true-to-life textile textures. The clothes should look like they were casually hung up in a real store, not perfectly arranged for a catalog.

[CRITICAL: Garment Fidelity]
Each garment MUST be an exact reproduction of its matching full photo and detail strip. Strictly preserve:
...
${buildDetailGuardrails()}

${buildAccessorySection(accessories, uploadedCount)}
...
Using the uploaded garments from the user's original garment photos, display them with natural gravity and relaxed drape. Show realistic fabric weight, gentle natural wrinkles, and true-to-life textile textures. The clothes should look like they were casually hung up in a real store, not perfectly arranged for a catalog.
...
function buildReferenceBoardMapping(uploadedCount: number, accessories?: AccessoryUploadInfo): string {
  const hasShoes = accessories?.hasShoesImage ?? false;
  const hasBag = accessories?.hasBagImage ?? false;

  let accessoryImageNote = "";
  if (hasShoes && hasBag) {
    const baseIndex = uploadedCount * 2 + 2;
    accessoryImageNote = `\n\n[IMPORTANT: Separate Accessory Reference Images]\nIn addition to Image 1, you are given two more separate high-resolution images:\n- Image ${baseIndex}: the exact SHOES to use in the scene. Reproduce them faithfully.\n- Image ${baseIndex + 1}: the exact BAG to use in the scene. Reproduce it faithfully.\nThese separate images show the accessories in full detail. Use them as the primary fidelity source for shoes and bag.`;
  } else if (hasShoes) {
    accessoryImageNote = `\n\n[IMPORTANT: Separate Accessory Reference Image]\nIn addition to Image 1, you are given one more separate high-resolution image:\n- Image ${uploadedCount * 2 + 2}: the exact SHOES to use in the scene. Reproduce them faithfully.\nThis separate image shows the shoes in full detail. Use it as the primary fidelity source for shoes.`;
  } else if (hasBag) {
    accessoryImageNote = `\n\n[IMPORTANT: Separate Accessory Reference Image]\nIn addition to Image 1, you are given one more separate high-resolution image:\n- Image ${uploadedCount * 2 + 2}: the exact BAG to use in the scene. Reproduce it faithfully.\nThis separate image shows the bag in full detail. Use it as the primary fidelity source for the bag.`;
  }

  return `${buildSinglePassReferenceMap(
    uploadedCount,
    "one split reference board. The left side shows garment overview tiles and the right side shows the boutique scene template.",
    accessories,
  )}

[Reference Board Mapping]
You are given one split reference board (Image 1):
...
function buildCommonPromptIntro(uploadedCount: number, accessories?: AccessoryUploadInfo): string {
  return `${COMMON_PROMPT_INTRO_BASE}

${buildReferenceBoardMapping(uploadedCount, accessories)}
...
Recreate the right-side boutique template scene with high fidelity:
- A warm off-white matte microcement wall with a subtle creamy tone and fine artistic mineral grain, paired with a pale French-limestone / travertine-style tile floor in a warm beige-cream tone. The wall texture must be visibly readable, not just implied: refined handcrafted microcement with soft cloud-like tonal variation, delicate trowel movement, subtle mineral grain, powdery plaster depth, and a lightly mottled artisanal surface rather than flat painted drywall. The floor should look like natural aged stone tiles in mixed rectangular sizes, laid in a classic staggered modular pattern like the reference images. Make the limestone texture clearly readable: visible fossil-like pores, soft mineral variation, lightly weathered stone texture, softly chalky matte depth, and thin understated grout lines that can still be perceived at a glance. Keep both wall and floor matte, refined, and quietly luxurious. Keep the scene immaculately clean and understated, with no visible baseboard, no skirting, and no trim
- One slim silver horizontal rod spanning almost the full width of the composition across the upper section
- The rod should feel positioned near a window side of the boutique so that natural daylight can travel across the scene
- Use only the exact number of wooden hangers needed for the uploaded garments in the final result
- One premium handbag anchor hanging directly from the left side of the rod, visually separate from the garments, with roughly three-quarters of the bag visible in frame rather than the entire bag fully shown
- One slightly tilted clipped editorial magazine page near the upper-right rod area, but pulled a bit inward toward the center rather than pushed to the far edge
- One small round pedestal table in the lower-left area with a refined top surface
- One pair of women's shoes placed on or around the round table, shifted slightly inward toward the center so the shoes sit closer to the clothing group
...
- The handbag anchor on the left side of the rod, the clipped editorial page on the right, and the round table with shoes at the lower-left are mandatory scene anchors and must remain visible
...
- There must be a visible natural light-and-shadow atmosphere in the final image, with soft window-cast light shapes traveling across the wall, garments, and floor
...
- The handbag must hang directly from the metal rod itself on the far-left side, using its own strap or hook point. Show only about three-quarters of the handbag in frame so it feels naturally cropped by the close composition. Never hang the bag on a clothing hanger, never share a hanger with a garment, and never place the bag on the round table
- The metal rod must remain one single continuous horizontal line behind the whole display. It must visibly continue past the handbag on the left side and must not appear broken, cropped, blocked, or replaced where the bag hangs
- Keep the shoes and jewelry display compact and elegant; they should support the composition without becoming the main subject. Nudge the shoes slightly inward toward the center so they visually connect better with the garments
...
- Keep the overall styling premium, restrained, quietly luxurious, and phone-photo realistic rather than luxury showroom glossy
- Keep one clearly readable patch of wall visible near the garments so the microcement texture can actually be seen in the final image

[Additional Garment Constraints]
- Keep the garments noticeably flatter and more presentation-ready than the raw source photo: reduce obvious wrinkles, fold marks, hanger dents, bubbling, and seam puckering as much as possible
- Preserve the original fabric texture with strong fidelity: keep the real weave, jacquard grain, denim grain, thickness, gloss level, and tactile material character from the uploaded garment
- The result should feel like real fabric that has been lightly steamed for display, not artificially airbrushed, waxy, plastic, or texture-smoothed
...
- The rod must remain visibly extended to the left of the handbag as one connected uninterrupted rod segment; do not crop away the left rod segment and do not let the bag visually replace, cover, or terminate the rod ending

[Photography & Lighting]
Shoot from a near-frontal eye-level boutique phone-photo angle with only a very slight offset. Push the framing in even closer, approximately 8x tighter than the previous medium framing, aiming for an ultra-tight close-up display composition like a premium in-store garment detail shot. The top and pants must dominate the image and take up almost all of the frame at first glance, so the viewer immediately sees the fabric, silhouette, buttons, seams, and construction. Reduce empty wall and empty floor to the bare minimum, but keep one clearly readable strip of floor at the bottom of frame so the limestone tile pattern, grout lines, and natural stone texture are still visibly identifiable. Also keep one readable wall patch near the garments so the microcement texture, mineral grain, and plaster movement remain visible at first glance. The composition should feel compressed, enlarged, and visually packed, with almost no unnecessary negative space. Keep the left handbag anchor, the upper-right clipped editorial page, and the lower-left round table styling still visible, but crop them tighter and compress them inward so they behave like very small supporting accents around the clothes. Pull the clipped editorial page slightly inward toward the center so it supports the composition without hugging the edge. Position the rod close to the top of frame and let the garments hang very large in frame, with hems approaching the lower portion of the composition. Keep the crop composed and premium: extremely tight, very focused, enlarged, and visually dense, not awkwardly clipped. Do not use a top-down angle, upward-looking angle, or dramatic perspective distortion. Use soft natural indoor daylight that feels like real window-adjacent ambient light in a boutique, not a fully lit studio wall. Let the scene feel close to a window so that the light on the garments, wall, and floor behaves like real physical daylight: naturally refracted through window glass, softly bounced by the interior, and gently diffused by the room before landing on surfaces. Let gentle window-frame light and shadow shapes fall naturally across the garments, wall, and floor in one continuous flow, similar to soft late-afternoon window light passing into the room. The light should travel naturally from wall to floor with a smooth transition, creating a coherent natural light path rather than isolated patches. This light-and-shadow feeling should be organic, subtle, and elegant rather than staged: soft-edged, semi-diffused, lightly broken up, and physically plausible, not sharp graphic bars or hard theatrical projections. Keep the overall white balance around 3500K: warm and soft, but still clean and controlled, without drifting into yellow-orange tungsten. The exposure should be clean and bright, but not perfectly uniform across the whole scene: allow a subtle natural brightness falloff and very gentle directional shading so the wall and floor feel like they are lit by real ambient daylight. Keep shadows soft and light-edged, with realistic gentle depth. No dramatic spotlight, no hard shadow, no moody darkness, and no yellow tungsten cast. Keep the light feeling like natural ambient boutique daylight, not a deliberate studio lighting setup or lightbox setup.

[Visual Goal]
The final result should feel like a tasteful high-end boutique display directly inspired by the template image: warm, airy, realistic, neatly styled, quietly luxurious, and highly suitable for product presentation while still looking like a real in-store photo.`,
    negativePrompt: `missing premium handbag anchor on the left side of the rod, handbag replaced by garment, handbag hanging on clothing hanger, handbag sharing hanger with garment, handbag placed on round table, entire handbag fully centered in frame, bag replacing left rod area, cropped left rod segment, broken rod, interrupted rod, disconnected rod segment, rod ending at handbag, rod hidden behind bag, missing left rod extension, missing clipped editorial magazine page, editorial page stuck to extreme edge, editorial page too far to the right edge, missing round pedestal table, missing women's shoes on the table, missing jewelry bust, shoes pushed too far outward, shoes disconnected from garment group, large empty wall, too much empty wall, too much empty foreground, too much floor showing, no readable floor strip, no readable wall texture patch, microcement texture not visible, wall texture unreadable, flat wall background, limestone texture not visible, grout lines not visible, floor material unreadable, weak stone texture, polished stone look, generous blank margins, wide border around garments, clothes not filling frame, garments too small in frame, tiny garments in frame, camera too far away, weak subject presence, distant wide shot, medium-wide composition, loose composition, overly pulled-back framing, loose crop, zoomed-out look, garments floating in empty space, flat painted wall, plain drywall wall, glossy wall paint, smooth plastic wall, faux luxury marble wall, reflective wall finish, purple decorative wall, seamless white floor, pure white seamless floor, same-color wall and floor, glossy marble floor, dark stone floor, busy patterned tiles, thick grout lines, cheap ceramic tiles, polished porcelain tiles, gray concrete floor, dark slate floor, high-gloss stone floor, black-and-white checker tiles, luxury showroom gloss, busy department store, visible shelves full of merchandise, stacked product boxes, mannequin body, human model, retail signage, cashier desk, oversized furniture, giant decor sculpture, floor mirror, sofa, armchair, bench, stool as main prop, bag and shoes too far from the garment group, round table moved to the center, cheap casual bag styling, garments not hanging on the rod, garment count mismatch, extra fourth garment, invented outerwear not uploaded, empty hangers, spare hangers, harsh yellow lighting, orange tungsten cast, wrong color temperature, dim scene, moody darkness, dramatic spotlight, over-lit studio effect, artificial fill light, obvious rim light, forced cinematic lighting, flat studio lighting, fully even wall lighting, lightbox lighting, seamless e-commerce studio light, isolated wall light patch, isolated floor light patch, broken light transition from wall to floor, physically impossible refraction, fake lens flare lighting, synthetic glow patches, hard sunbeam streaks, sharp window shadow bars, harsh blinds shadow, hard-edged geometric shadows, theatrical projection lighting, top-down angle, overhead angle, low-angle shot, upward-looking view, obvious wrinkles, hard crease lines, fold marks, hanger dents, seam puckering, bubbled fabric, crumpled garment surface, texture loss, over-smoothed fabric, waxy fabric, plastic fabric sheen, cheap curved baseboard, bulky rounded skirting, PVC baseboard, plastic baseboard molding`,
  },
];
...
export function buildHangoutfitPrompt({
  ...
  const sections = [
    ...
    buildCommonPromptIntro(uploadedCount, accessories),
    template.prompt,
