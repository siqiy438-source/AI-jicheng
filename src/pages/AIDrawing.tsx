...
const contentFrameworks = [
  ...
  },
  {
    ...
    name: "漫画故事",
    icon: "📖",
    prompt: "colorful warm hand-drawn comic strip, 6 panels in 3x2 grid: 1) bold title at top, 2) each panel has colored sub-topic header and vivid scene with 3-5 cute characters, 3) cream background with warm browns, oranges, yellows, 4) hand-drawn pencil texture, 5) speech bubbles and text labels, 6) rich scene details with props in every panel. Topic: {user_prompt}"
  },
  {
    id: "flowchart",
    name: "流程图",
      ...
      <div onClick={closeAllMenus}>
        ...
        {(isGenerating || generatedImage) && (
          <div className="glass-card rounded-xl md:rounded-2xl p-3 md:p-6">
            ...
            {isGenerating ? (
              <GeneratingLoader
                message={conversationHistory.length > 0 ? "正在根据你的要求修改中..." : "正在生成中..."}
              />
            ) : generatedImage ? (
              <div>
